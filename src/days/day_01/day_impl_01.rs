...
use crate::days::day_base::day_common;
...
impl day_trait for day {
	fn get_id_str() -> String {"day_01".to_string()}
	fn get_result_str() -> String {
		// Load file TODO: Move to common
		let read_path : PathBuf = fs::canonicalize("./src/days/day_01/input").expect("Error converting path.");
		let mut value : String = fs::read_to_string(read_path.as_path()).expect("Error reading.");
		// Process string
		return format!("{}\n{}",day::process_file_str(&value, false),day::process_file_str(&value, true));
	}
}
...
impl day{
	fn process_digits(str_value: &str, include_number_strings : bool) -> u32 {
		...
		assert!(first_digit.is_some(), "First digit of {} at least must be a non null value.", str_value);
		if(first_digit.is_none()) {
			return 0;
		}
		else {
