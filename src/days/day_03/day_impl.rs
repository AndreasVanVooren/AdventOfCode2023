...
impl DayTrait for Day {
    ...
    fn get_result_str() -> String {
        let mut grid =
            DayCommon::input_file_to_grid::<Day, GridElem, _>(Day::char_to_grid_elem_conv);
...
impl Day {
    ...
    fn check_and_fill_adjacent_symbols(da_griddy: &mut grid::Grid<GridElem>) {
        ...
        for main_row in 0..num_rows {
            ...
            for main_col in 0..num_cols {
                ...
                for adj_row in
                    max(main_row as i32 - 1, 0) as usize..=min(main_row + 1, num_rows - 1) as usize
                {
                    for adj_col in max(main_col as i32 - 1, 0) as usize
                        ..=min(main_col + 1, num_cols - 1) as usize
                    {
    ...
    fn process_part_1(da_griddy: &mut grid::Grid<GridElem>) -> Option<u32> {
        ...
        for main_row in 0..num_rows {
            ...
            for main_col in 0..num_cols {
                if !(da_griddy[main_row][main_col].is_digit()
                    && da_griddy[main_row][main_col].adjacent_to_symbol
                    && !da_griddy[main_row][main_col].handled)
                {
                    continue;
                }
            ...
            for main_col in 0..num_cols {
                    ...
                    max(main_row as i32 - 1, 0) as usize..=min(main_row + 1, num_rows - 1) as usize
                {
                    for adj_col in max(main_col as i32 - 1, 0) as usize
                        ..=min(main_col + 1, num_cols - 1) as usize
                    {
                        if adj_col == main_col && adj_row == main_row {
                            continue;
                        }
                        if da_griddy[adj_row][adj_col].handled {
                            continue;
                        }
                        if !da_griddy[adj_row][adj_col].is_digit() {
                            continue;
                        }
...
mod day_03_test {
    ...
    fn test_with_base_str() {
        ...
        let mut grid =
            DayCommon::input_str_to_grid::<GridElem, _>(&test_str, Day::char_to_grid_elem_conv);
        let part_num_sum: u32 = Day::process_part_1(&mut grid)
            .expect("Couldn't get part num sum, grid was probably empty");
    ...
    fn test_with_base_str_str_interp() {
        ...
        let mut grid =
            DayCommon::input_str_to_grid::<GridElem, _>(&test_str, Day::char_to_grid_elem_conv);
        let gear_num_sum: u32 = Day::process_part_2(&mut grid)
            .expect("Couldn't get gear num sum, grid was probably empty");
